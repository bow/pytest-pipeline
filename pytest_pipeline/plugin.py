...
def _autogenerated_test_dir(request):
    run_dir = request.cls.test_dirname
    init_dir = os.getcwd()
    test_dir = os.path.join(request.config.option.base_pipeline_dir, run_dir)
...
def pytest_collection_modifyitems(session, config, items):
    ...
    class_order = {grouped[0]: i for i, grouped in
                   enumerate(groupby(items, key=lambda item: item.cls.__name__))}
